...
@fig-model1ppcheck shows the posterior predictive checks for the first model, which predicts Trump's percentage vote over time in the national perspective. 
...
#| echo: false
#| eval: true
#| label: fig-model1ppcheck
#| fig-cap: "Posterior predictive checks for model 1 (national)"
#| warning: false
...
In @fig-model1ppcheck, the black line basically follows the trend of the light blue lines, indicating the observed data consistently falling within the range of simulated data. This suggests that the model provides a good fit, effectively predicting the actual trend of the data.

Similarly, @fig-model2ppcheck shows the posterior predictive checks for the second model, which predicts Trump's percentage vote over time by state.


```{r}
#| echo: false
#| eval: true
#| label: fig-model2ppcheck
#| fig-cap: "Posterior predictive checks for model 2 (by state)"
#| warning: false
...
In @fig-model2ppcheck, Although the black line has minor deviation away from the light blue lines, and a part of the black line falls outside the range of simulated lines, the majority part of the black line matches with the light blue lines. 
...
## Predictions and spline fit for vote percentage nationally
...
3. Online Panels: 
Emerson also utilizes verified online panels, where respondents’ eligibility is cross-checked with voter file data, in order to exclude not potential voter’s response, this step reinforces sample accuracy.
...
Non-probability sampling means not all individuals have an equal chance of being selected, in some of Emerson’s methods such as landline call, targeted people are not randomly choose, which can increase potential bias.
...
1. Simplicity: 
Online surveys typically rely on multiple-choice or straightforward questions, which may limit deeper insights into complex opinions or voter’s tendency. 
